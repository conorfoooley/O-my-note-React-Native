...
class NoteSelection extends React.PureComponent<Props, State> {
  ...
  compressAndUploadStory = async () => {
    const { videoPath, videoType, video, textStory } = this.props.navigation.state
    ...
    if (videoType === 'library' && Platform.OS === 'android') {
      this.storyUpload(story, token, video, videoPath);
      ...
      this.storyUpload(story, token, video, compressedVideoPath);
    }
  };
  ...
  storyUpload = (story: any, token: string, video?: string, videoPath?: string) => {
    try {
      if (videoPath) {
        ...
        story.append('language', getPreferredLocale());
